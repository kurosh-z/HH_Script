...
class Geologger(object):
    ...
    def init(self, logPath, *args, **kwargs):
        ...
        try:
            ...
            raise (
                "Oops!",
                "occurred. if you run this for the first time "
    ...
    def generate_date_list(self, hhtype):
        dates = [self.logJSON[hhtype][i]["date"] for i in range(len(self.logJSON[hhtype]))]
        return dates
    ...
    def add_new_gas_entry(self, date, gas, hhtype):
        ...
        if not date_exist:
            raise Exception('the date: "{}" ,which you want to add gas entry does not exist!'.format(date))
        self.logJSON[hhtype][dateIndex]["data"].append({"gas": gas, "gasInfo": []})
    ...
    def list_all_fileName_in_gasInfo(self, date, gas, hhtype):
        ...
        if not date_exist:
            raise Exception('there exist no date: "{}" ,in which you want to list all fileNames!'.format(date))
        if not gas_exist:
            raise Exception(
    ...
    def add_new_gasInfo(self, date, gas, gasInfo, hhtype):
        ...
        self.logJSON[hhtype][dateIndex]["data"][gasIndex]["gasInfo"].append(gasInfo)
        #     {'gas': gas, 'gasInfo': []})
